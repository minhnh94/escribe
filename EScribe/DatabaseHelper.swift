...
class DatabaseHelper: NSObject {
    ...
    func loadAllCompletedPatientNotes(patientId: Int) -> [PatientNote] {
        ...
        arrayPatientNotes.sort { (noteA, noteB) -> Bool in
            noteA.datetime > noteB.datetime
        }
    ...
    func createNewPatientNote(patient: Patient, loadFromPatientNoteId: Int) -> Int {
        ...
        if patient.internalId == 0 {
            ...
            let rowId = try! db.run(patientNoteTable.insert(patientId <- returnedId, author <- VariousHelper.shared.loadCurrentPhysicianName(), datetime <- VariousHelper.shared.getDateAndTimeTodayAsString(), kStoredType <- 0, kVoiceRecIndex <- 0))
            return Int(rowId)
        }
        
        // Else do this
        let rowId = try! db.run(patientNoteTable.insert(patientId <- patient.internalId, author <- VariousHelper.shared.loadCurrentPhysicianName(), datetime <- VariousHelper.shared.getDateAndTimeTodayAsString(), kStoredType <- 0, kVoiceRecIndex <- 0))
        return Int(rowId)
    }
